...
const Signup: React.FC<SignupProps> = ({ onSignup, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Handle Google signup with company name
    if (showCompanyPrompt && googleCredential) {
      try {
        ...
        setError(err.error || "Google signup failed");
      } finally {
        setIsLoading(false);
      }
      return;
  ...
  const initiateGoogleSignIn = () => {
    ...
    window.google.accounts.id.initialize({
      client_id: clientId,
      callback: (response: any) => handleGoogleSignup(response.credential),
    });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        const temp = document.getElementById("google-signin-button-temp");
    ...
    <div className="min-h-screen bg-white flex font-sans">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-indigo-900 text-white flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          >
            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center font-bold text-indigo-900">
              C
            </div>
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <ul className="space-y-4 text-indigo-100 text-lg">
            <li className="flex items-center gap-3">
              <span className="w-6 h-6 rounded-full bg-green-400 flex items-center justify-center text-indigo-900 font-bold text-sm">
                ✓
              </span>{" "}
              Unified Inbox
            </li>
        ...
        <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold text-slate-900">
              {showCompanyPrompt ? "One more thing..." : "Create your account"}
            </h1>
            <p className="text-slate-500 mt-2">
              {showCompanyPrompt
                ? "What's your company name?"
                : "Get started with ConnectFlo for free."}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {showCompanyPrompt ? (
              // Simplified form for Google signup
              <>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      className="w-full pl-9 pr-10 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                      placeholder="••••••••"
                      required
                    />
                    <button
