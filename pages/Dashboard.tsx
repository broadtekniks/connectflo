...
import { MOCK_METRICS, MOCK_AGENT_LEADERBOARD, MOCK_SENTIMENT_HISTORY, MOCK_VOICE_COSTS, MOCK_INTENTS, MOCK_CONVERSATIONS } from '../constants';
import { ArrowUp, ArrowDown, Minus, Phone, Zap, Users, LayoutDashboard, TrendingUp, DollarSign, Activity, Clock, AlertCircle, ArrowRight, CheckCircle } from 'lucide-react';
...
const MetricCard: React.FC<{ label: string; value: string | number; trend?: string; change?: number; icon?: React.ReactNode }> = ({ label, value, trend, change, icon }) => (
...
const Dashboard: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'voice' | 'ai' | 'agents'>('overview');
             ...
             <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6">
                    ...
                    <div className="flex gap-2 mt-4 md:mt-0">
                        ...
                        <button className="bg-indigo-600 text-white text-sm font-medium px-4 py-2 rounded-md hover:bg-indigo-700 transition-colors shadow-sm">Export Report</button>
                    </div>
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-8">
            <div className="max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="space-y-8 animate-fade-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {MOCK_METRICS.map((metric, index) => (
                                <MetricCard key={index} {...metric} icon={<Activity size={20}/>} />
                            ))}
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h3 className="font-bold text-slate-800 mb-6">Channel Volume</h3>
                                <div className="h-80 w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={overviewData}>
                                            ...
                                            <Legend />
                                            <Bar dataKey="chat" stackId="a" fill="#6366f1" radius={[0, 0, 4, 4]} name="Chat" />
                                            <Bar dataKey="voice" stackId="a" fill="#a5b4fc" radius={[4, 4, 0, 0]} name="Voice" />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                                <h3 className="font-bold text-slate-800 mb-2">Automation Rate</h3>
                                <p className="text-sm text-slate-500 mb-6">Sessions handled by AI vs Agents</p>
                                <div className="flex-1 min-h-[200px] relative">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                                        ...
                                        <span className="text-xs text-slate-400 font-medium uppercase">Automated</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="divide-y divide-slate-100">
                                    {urgentConversations.length > 0 ? (
                                        urgentConversations.map(conv => (
                                            <div key={conv.id} className="p-5 hover:bg-slate-50 transition-colors flex items-center gap-4 group cursor-pointer">
                ...
                {activeTab === 'voice' && (
                    <div className="space-y-8 animate-fade-in">
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <MetricCard label="Total Voice Minutes" value="3,450" change={12} trend="UP" icon={<Phone size={20}/>} />
                            <MetricCard label="Avg Handle Time" value="4m 12s" change={-5} trend="DOWN" icon={<Clock size={20}/>} />
                            <MetricCard label="Telephony Cost" value="$145.20" change={8} trend="UP" icon={<DollarSign size={20}/>} />
                        </div>

                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h3 className="font-bold text-slate-800 mb-6">Daily Cost & Usage</h3>
                                <div className="h-80 w-full">
                                     <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={MOCK_VOICE_COSTS}>
                                            ...
                                            <Tooltip />
                                            <Legend />
                                            <Bar yAxisId="left" dataKey="cost" name="Cost ($)" fill="#6366f1" radius={[4, 4, 0, 0]} />
                                            <Bar yAxisId="right" dataKey="minutes" name="Minutes" fill="#e2e8f0" radius={[4, 4, 0, 0]} />
                                        </BarChart>
                                     </ResponsiveContainer>
                                </div>
                            </div>
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h3 className="font-bold text-slate-800 mb-2">Abandonment Rate</h3>
                                <p className="text-sm text-slate-500 mb-6">Calls dropped before reaching an agent</p>
                                <div className="flex items-center justify-center h-64">
                                    <div className="text-center">
                                        <div className="w-40 h-40 rounded-full border-8 border-slate-100 border-t-red-500 flex items-center justify-center mx-auto mb-4">
                                            <span className="text-3xl font-bold text-slate-800">4.2%</span>
                                        </div>
                                        <p className="text-sm text-slate-500">Below industry average (5.0%)</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {activeTab === 'ai' && (
                     <div className="space-y-8 animate-fade-in">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <MetricCard label="Tokens Used" value="2.4M" change={25} trend="UP" icon={<Zap size={20}/>} />
                            <MetricCard label="Deflection Rate" value="68%" change={2} trend="UP" icon={<TrendingUp size={20}/>} />
                            <MetricCard label="Est. Cost Savings" value="$4,200" change={15} trend="UP" icon={<DollarSign size={20}/>} />
                        </div>

                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h3 className="font-bold text-slate-800 mb-6">Sentiment Trends (Today)</h3>
                                <div className="h-80 w-full">
                                     <ResponsiveContainer width="100%" height="100%">
                            ...
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h3 className="font-bold text-slate-800 mb-6">Top Detected Intents</h3>
                                <div className="h-80 w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart layout="vertical" data={MOCK_INTENTS} margin={{ left: 40 }}>
                                            <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                                            <XAxis type="number" />
                                            <YAxis dataKey="name" type="category" width={100} tick={{fontSize: 12}} />
                                            <Tooltip />
                                            <Bar dataKey="value" fill="#6366f1" radius={[0, 4, 4, 0]} barSize={20} name="Occurrences" />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                             ...
                             <table className="w-full text-left text-sm">
                                <thead className="bg-white text-slate-500 border-b border-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 font-semibold">Resolved Tickets</th>
                                        <th className="px-6 py-3 font-semibold">CSAT Score</th>
                                        <th className="px-6 py-3 font-semibold">Avg Handle Time</th>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {MOCK_AGENT_LEADERBOARD.map((agent) => (
                                        <tr key={agent.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 font-medium">{agent.resolved}</td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <span className={`font-bold ${agent.csat >= 4.8 ? 'text-green-600' : 'text-slate-600'}`}>{agent.csat}</span>
                                                    <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                                        <div className="h-full bg-indigo-500 rounded-full" style={{width: `${(agent.csat / 5) * 100}%`}}></div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-slate-500">{agent.avgTime}</td>
