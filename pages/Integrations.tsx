...
import { Check, Plus, Trash2, Search, Box } from 'lucide-react';
...
const IntegrationIcon: React.FC<{ integration: Integration }> = ({ integration }) => {
    ...
    <div className="flex-1 bg-slate-50 p-8 overflow-y-auto h-full">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
            ...
            <p className="text-slate-500 mt-1">Connect your tools to power agent workflows and AI responses.</p>
        </div>
            ...
            <div className="flex-1">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredIntegrations.map(int => (
                        <div key={int.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                {int.connected ? (
                                    ...
                                    <button className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full hover:bg-indigo-100 transition-colors">
                                        Connect
                                    </button>
                                )}
                            </div>
                            <div className="mb-2">
                                ...
                                <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wide">{int.category}</span>
                            </div>
                            <p className="text-xs text-slate-600 mb-4 flex-1 leading-relaxed">{int.description}</p>
                            {int.connected && (
                                <div className="flex justify-between items-center pt-3 border-t border-slate-100 mt-auto">
                                    <button className="text-xs font-medium text-slate-500 hover:text-indigo-600">Configure</button>
                                    <button className="text-slate-400 hover:text-red-500 transition-colors" title="Disconnect">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            )}
